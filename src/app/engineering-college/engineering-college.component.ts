...
@Component({
  selector: 'app-engineering-college',
  templateUrl: './engineering-college.component.html',
  styleUrls: ['./engineering-college.component.css']
})
export class EngineeringCollegeComponent implements OnInit {
  animal: string;
  name: string;
  Fees: any = [];
  Facilities: any = [];
  ...
  ngOnInit() {
    ...
    this._httpClient.post(`${this.apiUrl}/authApi/getcollege`, data).subscribe((response: any) => {
      if (response.Status == 200) {
        this.spinner.hide();
        this.TopEngineering = response.result;
      } else {
        this.TopEngineering = [];
      }
    })
